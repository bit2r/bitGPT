...
#' Retrieve models
#' @description 모델 인스턴스를 검색하여 소유자 및 권한 부여와 같은 모델에 대한 기본 정보를 제공.
#' @param model character. 조회할 모델의 이름.
#' @return a data.frame.
#' Names of data.frame is as follows.
#' \itemize{
#'   \item id : 모델의 고유 식별자. 이 식별자는 모델을 호출하거나 다른 API 요청에서 참조할 때 사용됨.
...
#' @importFrom openai list_models
#' @importFrom lubridate as_datetime
#' @importFrom purrr map_dfr modify_if
#' @importFrom tibble as_tibble
retrieve_model <- function(model) {
  model <- openai::retrieve_model(model) %>%
